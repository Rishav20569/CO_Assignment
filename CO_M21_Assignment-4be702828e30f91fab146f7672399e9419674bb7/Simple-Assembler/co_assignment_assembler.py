...
#to store instruction address
Address=[]
#stores variables
variable=[]

#stores variable count or address
Var={}

# function to convert decimal to binary of 8bits
def Bin(x):
    b=bin(x)[2:]
    a=(8-len(b))*"0"+str(b)
    return a
...
for i in range(0,len(variable)-1):
    ...
    if variable[i]!="var":
        if variable[i+1]=="var":
            print("Line "+ str(add_count-1)+":"+" Variables not declared at beginning.")
            flag=1
            break

#adding labels in this list
Label_add={}
c=-1
...
if add_count>256:
    ...
    for line in assemblyfile:
        ...
        if x[0]=="var":
            if(len(x)==1):
                print("Line"+" "+str(add_count-1)+":"+"General syntax error")
                flag=1
        else:
            Address.append(add_count)
            ...
            if x[0][-1]==":":
                    ...
                    if len(x)==4:
                        ...
                        if(x[1] not in REG_ADD.keys() or x[2] not in REG_ADD.keys() or x[3] not in REG_ADD.keys()):
                            ...
                            if x[1]=="FLAGS" or x[2]=="FLAGS" or x[3]=="FLAGS":
                                print("Line"+" "+str(add_count-1)+":"+"Illegal use of FLAGS register.")
                            else:
                                print("Line"+" "+str(add_count-1)+":"+"Typos in register name.")
                            break
                ...
                if (x[0]=="mov" and x[2][0]=="$") or x[0]=="rs" or x[0]=="ls":
                    if (x[1] in REG_ADD.keys()):
                        ...
                        if(z<0 or z>255):
                            flag=1
                            print("Line"+" "+str(add_count-1)+":"+"Illegal immediate values.")
                            break
                    else:
                        flag=1
                        if x[1]=="FLAGS" :
                ...
                if x[0]=="ld" or x[0]=="st":
                    ...
                    if(x[2] not in Var.keys()):
                        flag=1
                        print("Line"+" "+str(add_count-1)+":"+"Use of undefined variables.")
                        break
                ...
                if x[0]=="jmp" or x[0]=="jlt" or x[0]=="jgt" or x[0]=="je":
                    ...
                    if((x[1] +":") not in Label_add.keys()):
                        flag=1
                        print("Line"+" "+str(add_count-1)+":"+"Use of undefined labels.")
                        break
                else:           
                    if (x[0] not in Instruction.keys()):
                        print("Line"+" "+str(add_count-1)+":"+"Typos in instruction name.")
                        break
                
                #for Type F instruction
                if x[0]=="hlt" and c!=len(assemblyfile):
                    print("Line"+" "+str(add_count-1)+":"+"hlt must be at the end")
                    flag=1
